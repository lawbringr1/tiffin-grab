#!/usr/bin/env python3
"""
Create (once) or update the TiffinGrab Catering Elementor page.

First run (no map file):
  - elementor-mcp-create-page -> new published page
  - Full-width container + HTML widget with catering-page-2026.html
  - Writes elementor-html/.catering-page-element-id.json

Later runs (map file exists):
  - Only updates the HTML widget content (same post_id / html_widget)

Header (1863) and footer (1907) come from the site Theme Builder, so the
widget carries page body only.

Requires: .cursor/mcp.json with elementor-mcp url + Authorization.
TLS: prefix with SSL_CERT_FILE=/etc/ssl/cert.pem (system python lacks CA certs).

Usage:
  SSL_CERT_FILE=/etc/ssl/cert.pem python3 scripts/setup_catering_page_2026.py
"""
...
HTML_FILE = ROOT / "elementor-html" / "catering-page-2026.html"
OUT_MAP = ROOT / "elementor-html" / ".catering-page-element-id.json"
PAGE_TITLE = "Catering"
...
def mcp_init(url: str, auth: str) -> str:
    body = {
        ...
        "params": {
            ...
            "capabilities": {},
            "clientInfo": {"name": "setup-catering", "version": "1"},
        },
    }
...
def main() -> None:
    ...
    print("Created Catering page post_id:", post_id, "html_widget:", wid)
