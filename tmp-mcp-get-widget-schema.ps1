param([Parameter(Mandatory = $true)][string]$WidgetType)
...
$bodyObj = @{
  ...
  method = "tools/call"
  params = @{
    name = "elementor-mcp-get-widget-schema"
    arguments = @{ widget_type = $WidgetType }
  }
}
$json = $bodyObj | ConvertTo-Json -Depth 20 -Compress
$payloadPath = Join-Path $PSScriptRoot ".tmp-mcp-widget-schema-payload.json"
